...
export ANSIBLE_CONFIG=$current_directory/ansible.cfg
arg1=$1
...
AnsibleExec() {
    ...
    if ${ANSIBLE} all --list-hosts -i $current_directory/inventory | grep -q '(0)' 2>/dev/null; then
    ...
    echo ""
    ${ANSIBLE}-playbook $verbose $current_directory/omnileads.yml \
      --extra-vars "repo_location=$REPO_LOCATION \
                    docker_root=$USER_HOME \
                    build_images=$BUILD_IMAGES \
                    oml_release=$release_name \
                    commit=$commit \
                    build_date=\"$build_date\"" \
      --tags "$tag" -i $current_directory/inventory
