...
from api_app.serializers import (
    CampanaSerializer, AgenteDeCampanaSerializer,
    ...
    def get(self, request, pk_campana):
        ...
        try:
            ...
            if self._is_valid_campaign(campana):
                ...
                data['agents_campaign'] = [AgenteDeCampanaSerializer(qm).data for qm in queue_members]
            ...
            try:
                ...
                for agent_id in agent_ids_to_delete:
                    try:
                        ...
                        data['status'] = 'ERROR'
                        data['message'] = _(u'No existe el agente, no se puede eliminar de la member queue')
                        return Response(data=data, status=status.HTTP_404_NOT_FOUND)
                ...
                for agent in agents:
                    ...
                    try:
                        ...
                        data['status'] = 'ERROR'
                        data['message'] = _(u'No existe el agente, no se puede crear la member queue')
                        return Response(data=data, status=status.HTTP_404_NOT_FOUND)
...
class AgentesActivosPorGrupo(APIView):
    permission_classes = (TienePermisoOML, )
    authentication_classes = (SessionAuthentication, ExpiringTokenAuthentication, )
    renderer_classes = (JSONRenderer, )
    http_method_names = ['get']
    ...
    def get(self, request):
        data = {
            'status': 'SUCCESS',
            'message': _(u'Se obtuvieron los grupos con agentes de forma exitosa'),
            'groups': []}
        try:
