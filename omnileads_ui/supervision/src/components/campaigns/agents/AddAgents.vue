...
export default {
  ...
  methods: {
    updateAgentsDisplay() {
      const agents_campaign_ids = this.agents_by_campaign.map(
        (agent) => agent["agent_id"]
      );
      const active_agents_filter = this.active_agents.filter(
        (agent) => !agents_campaign_ids.includes(agent["agent_id"])
      );
      this.agents = active_agents_filter.map((agent) => {
        return {
